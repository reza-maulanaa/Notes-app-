import { signIn } from "@/auth";
import { loginAction } from "../login/action";
import Link from "next/link";
import { PenLine } from "lucide-react";

interface Props {
  searchParams: Promise<{ error?: string; verified?: string }>;
}

async function githubAction() {
  "use server";
  await signIn("github", { redirectTo: "/notes" });
}

async function googleAction() {
  "use server";
  await signIn("google", { redirectTo: "/notes" });
}

export default async function LoginPage({ searchParams }: Props) {
  const { error, verified } = await searchParams;

  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--color-surface)] px-4">
      {/* Subtle background pattern */}
      <div
        className="pointer-events-none fixed inset-0 opacity-[0.35]"
        style={{
          backgroundImage:
            "radial-gradient(circle, oklch(0.700 0.008 28) 1px, transparent 1px)",
          backgroundSize: "28px 28px",
        }}
      />

      <div className="relative z-10 w-full max-w-sm">
        {/* Logo */}
        <div className="mb-8 flex flex-col items-center gap-3">
          <Link href="/" className="flex items-center gap-2">
            <div
              className="flex h-8 w-8 items-center justify-center rounded-lg"
              style={{ backgroundColor: "var(--color-ink)" }}
            >
              <PenLine size={15} className="text-white" />
            </div>
            <span className="text-sm font-semibold text-[var(--color-ink)]">
              Notes
            </span>
          </Link>
          <div className="text-center">
            <h1 className="text-xl font-bold tracking-tight text-[var(--color-ink)]">
              Selamat datang kembali
            </h1>
            <p className="mt-1 text-sm text-[var(--color-muted)]">
              Masuk untuk melihat catatanmu
            </p>
          </div>
        </div>

        {/* Card */}
        <div className="rounded-2xl border border-[var(--color-border)] bg-white p-6 shadow-sm">
          {/* Error messages */}
          {error === "too_many_attempts" && (
            <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-xs text-red-700">
              Terlalu banyak percobaan. Coba lagi dalam 15 menit.
            </div>
          )}
          {error === "invalid_credentials" && (
            <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-xs text-red-700">
              Email atau password salah.
            </div>
          )}

          {/* Sukses verifikasi */}
          {verified === "true" && (
            <div style={{ color: "green", marginBottom: "12px" }}>
              Email berhasil diverifikasi! Silakan login.
            </div>
          )}

          {/* Token expired */}
          {error === "token_expired" && (
            <div style={{ color: "red", marginBottom: "12px" }}>
              Link verifikasi sudah expired. Daftar ulang untuk mendapat link
              baru.
            </div>
          )}

          {/* Social auth */}
          <div className="mb-4 grid grid-cols-2 gap-2">
            <form action={githubAction}>
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-ink)] px-3 py-2.5 text-xs font-medium text-white transition-all hover:opacity-90 active:scale-[0.98] cursor-pointer"
              >
                <svg
                  viewBox="0 0 24 24"
                  className="h-4 w-4 fill-current"
                  aria-hidden="true"
                >
                  <path d="M12 0C5.37 0 0 5.373 0 12c0 5.303 3.438 9.8 8.205 11.387.6.113.82-.258.82-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" />
                </svg>
                GitHub
              </button>
            </form>
            <form action={googleAction}>
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 rounded-lg border border-[var(--color-border)] bg-white px-3 py-2.5 text-xs font-medium text-[var(--color-ink)] transition-all hover:bg-[var(--color-surface)] active:scale-[0.98] cursor-pointer"
              >
                <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
                  <path
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                    fill="#4285F4"
                  />
                  <path
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    fill="#34A853"
                  />
                  <path
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    fill="#FBBC05"
                  />
                  <path
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    fill="#EA4335"
                  />
                </svg>
                Google
              </button>
            </form>
          </div>

          {/* Divider */}
          <div className="relative my-4">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-[var(--color-border)]" />
            </div>
            <div className="relative flex justify-center">
              <span className="bg-white px-3 text-xs text-[var(--color-muted)]">
                atau dengan email
              </span>
            </div>
          </div>

          {/* Credentials form */}
          <form action={loginAction} className="space-y-3">
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[var(--color-ink)]">
                Email
              </label>
              <input
                name="email"
                type="email"
                required
                autoComplete="email"
                placeholder="kamu@email.com"
                className="w-full rounded-lg border border-[var(--color-border)] bg-white px-3 py-2.5 text-sm text-[var(--color-ink)] placeholder:text-[var(--color-muted)] transition-colors focus:border-[var(--color-ink)] focus:outline-none"
              />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[var(--color-ink)]">
                Password
              </label>
              <input
                name="password"
                type="password"
                required
                autoComplete="current-password"
                placeholder="••••••••"
                className="w-full rounded-lg border border-[var(--color-border)] bg-white px-3 py-2.5 text-sm text-[var(--color-ink)] placeholder:text-[var(--color-muted)] transition-colors focus:border-[var(--color-ink)] focus:outline-none"
              />
            </div>
            <button
              type="submit"
              className="mt-1 w-full rounded-lg py-2.5 text-sm font-semibold text-white transition-all hover:opacity-90 active:scale-[0.98] cursor-pointer"
              style={{ backgroundColor: "var(--color-ink)" }}
            >
              Masuk
            </button>
          </form>
        </div>

        <p className="mt-5 text-center text-xs text-[var(--color-muted)]">
          Belum punya akun?{" "}
          <Link
            href="/register"
            className="font-medium text-[var(--color-ink)] hover:underline"
          >
            Daftar gratis
          </Link>
        </p>
      </div>
    </div>
  );
}
